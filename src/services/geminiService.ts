...
Return ONLY a valid JSON array containing objects for each lifecycle stage. No additional text or formatting.
  ...
  async assessLifecycle(input: LCAInput): Promise<LCAStageResponse[]> {
    try {
      ...
      try {
        const parsedResponse: LCAStageResponse[] = JSON.parse(cleanedText);
      ...
      throw new Error(
        `Failed to generate LCA assessment: ${
  ...
  private getFallbackResponse(input: LCAInput): LCAStageResponse[] {
    return [
      {
        stage: "Raw Material Extraction/Mining",
        impact: {
          carbon_emission:
            "Estimated 2-5 tCO2eq per ton of material (hypothetical)",
        ...
        main_cause:
          "Heavy machinery operations and ore processing require significant energy",
        alternative_methods: [
      ...
      },
      {
        ...
        main_cause:
          "Energy-intensive crushing, grinding, and separation processes",
        alternative_methods: [
